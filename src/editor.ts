...
import { Bold } from './toolbars/bold';
...
export class Editor {
  ...
  protected tool_element_: HTMLElement;
  ...
  constructor (elementContainer_: HTMLElement, config: EditorConfigProps = {}) {
    ...
    // this.initToolbarsElement();
    this.initElement_(); // 初始化创建可编辑的pre元素
    this.textModel_ = new TextModel(); // 初始化内容文本模型
    this.selectionModel_ = new SelectionModel(this.textModel_); // 初始化选区模型
    this.viewProvider_ = new ViewProvider();
    this.initView_();
    // this.initToolbarsView()
  }

  private initElement_() {
    const element = document.createElement("pre");
    element.setAttribute("placeholder", this.config_.placeholder);
    element.setAttribute("contenteditable", "true"); //设置元素允许被用户编辑
  ...
  private initToolbarsElement() {
    // const toolbars= new Toolbars()
    // toolbars.append(new Bold().element)
    // this.tool_element_ = toolbars.element
    // this.elementContainer_.append(toolbars.element)
    // const element = document.createElement("div");
    // element.setAttribute("style","display: flex; justify-content: center; align-items:center; flex-wrap: wrap;")
  }
   ...
   */
  getLineCusor() {
    const anchor = this.getSelectionModel().getSelection().anchor
    let start = 0
    let end = this.getContent().length
    if (this.getContent().substring(0, anchor).indexOf("\n") > -1) {
      start = this.getContent().substring(0, anchor).lastIndexOf("\n")+1
    }
    return start
  }
  ...
  getLineModel() {
    ...
    if (this.getContent().substring(anchor).indexOf("\n") > -1) {
      end = this.getContent().substring(anchor).indexOf("\n")
    }
    return this.getContent().substring(start,anchor+end)
  }
  ...
  getLineCusorFront() {
    ...
    return this.textModel_.getLineByIndex(selection.anchor)
  }
